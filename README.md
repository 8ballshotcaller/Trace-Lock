...
lay paper over your phone and trace without the image sliding around. Holding
down the bottom-right corner for 5 seconds is the only way to unlock it
again.

## Features

- Drag & drop / tap-to-choose / paste an image
- Pinch-to-zoom, drag-to-pan, rotate 90°, horizontal flip, fade (opacity)
  slider, invert colors
- Double-tap the image to snap back to fit; arrow keys nudge it a pixel (or
  10px with Shift) for fine-tuned positioning
- Keeps the screen awake the whole time you're positioning or tracing, so it
  won't dim or lock mid-session (where the browser supports the Wake Lock
  API — most current Android/desktop browsers and Safari 16.4+)
- **Freeze & Trace mode**: blocks touchmove, pinch-zoom, long-press menus,
  and drag — the screen genuinely won't move
- Unlock by holding down the bottom-right corner for 5 seconds (a ring fills
  around a small timer icon as you hold) — no accidental unlocks from stray
  tracing touches
...
3. Tap **🔒 Freeze & Trace**
4. Lay paper over the screen and trace — the screen will not move no matter
   how much you press on it
5. To unlock: **hold down** the bottom-right corner of the screen for
   5 seconds — a ring fills clockwise around the timer icon as you hold
...
├── app.js             # all logic (upload, pan/zoom, freeze, hold-to-unlock)
...
- Images aren't saved between sessions on purpose (nothing is written to
  disk/server) — only your fade/invert preferences are remembered locally on
  your device.
- Long-press-to-save-image and pinch-zoom-out-of-the-page are also blocked
  while frozen, along with normal scrolling.
- There's no code to forget — unlocking only ever requires the 5-second
  bottom-right hold.
