(() => {
  ...
  function isIOSSafari() {
    // True mobile Safari only — excludes Chrome/Firefox/Edge/Opera "iOS"
    // builds (all Safari under the hood, but none of them expose the
    // Share-sheet "Add to Home Screen" action) and in-app webviews
    // (Instagram/TikTok/Gmail/etc. link previews).
    const ua = navigator.userAgent;
    const isOtherBrowser = /CriOS|FxiOS|EdgiOS|OPiOS|OPT\/|GSA\//.test(ua);
    const isWebView = /\b\w+\/[\d.]+ like Mac OS X\)\s*(?!.*Version\/)/.test(ua) ||
      (/\bMobile\/\w+/.test(ua) && !/Version\//.test(ua));
    return isIOS() && !isOtherBrowser && !isWebView;
  }
  ...
  if (isIOS() && !isStandalone() && !localStorage.getItem(IOS_HINT_KEY)) {
    if (!isIOSSafari()) iosHint.classList.add("not-safari");
