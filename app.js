(() => {
  ...
  const unlockLeft  = document.getElementById("unlock-left");
  ...
  /* ---------------- Two-corner hold-to-unlock ---------------- */
  const HOLD_MS = 1500;
  let leftDown = false, rightDown = false;
  let holdStart = null, holdRAF = null;

  function setProgress(p) {
    unlockLeft.querySelector(".unlock-ring").style.setProperty("--p", p);
  ...
  function maybeStartHold() {
    if (leftDown && rightDown && holdRAF === null) {
      holdStart = null;
      holdRAF = requestAnimationFrame(tick);
    }
  }
  ...
  function unlock() {
    ...
    unlockLeft.classList.remove("pressed");
    unlockRight.classList.remove("pressed");
    exitFrozenMode();
    showScreen(screenEditor);
  }

  function bindZone(el, setDown) {
    const down = (e) => {
      e.preventDefault();
      setDown(true);
  ...
  bindZone(unlockLeft,  (v) => { leftDown = v; });
  bindZone(unlockRight, (v) => { rightDown = v; });
